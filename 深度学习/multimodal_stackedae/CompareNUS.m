...
cq_hiddenSizeL1 = 120;    % Layer 1 Hidden Size
cq_hiddenSizeL2 = 50;    % Layer 2 Hidden Size

lss_hiddenSizeL1 = 30;    % Layer 1 Hidden Size
lss_hiddenSizeL2 = 30;    % Layer 2 Hidden Size

phog_hiddenSizeL1 = 110;    % Layer 1 Hidden Size
phog_hiddenSizeL2 = 40;    % Layer 2 Hidden Size

sift_hiddenSizeL1 = 110;    % Layer 1 Hidden Size
sift_hiddenSizeL2 = 40;    % Layer 2 Hidden Size

surf_hiddenSizeL1 = 50;    % Layer 1 Hidden Size
surf_hiddenSizeL2 = 40;    % Layer 2 Hidden Size
...
%%======================================================================
%% STEP 1: Load data from the NUS database
% Load NUS database files
load '..\MMSSL\NUS\Dataset_CH.mat'
load '..\MMSSL\NUS\Dataset_CM55.mat'  
load '..\MMSSL\NUS\Dataset_EDH.mat'  
load '..\MMSSL\NUS\Dataset_CORR.mat'  
load '..\MMSSL\NUS\Dataset_WT.mat'

tic;

% 15 classes, every class has 500 samples
classsize = 500;
select_data  = 100; % every experiment select samples
train_samples = select_data * 0.8;
test_samples = select_data * 0.2;
sample_num = select_data * numClasses;

label = zeros(size(Dataset_CH,1),1);

for i = 1:numClasses
    label((i-1)*classsize+1:i*classsize) = i*ones(classsize,1);
end
    
%label = [10*ones(1,500) ones(1,500) 2*ones(1,500) 3*ones(1,200) 4*ones(1,200) 5*ones(1,200) 6*ones(1,200) 7*ones(1,200) 8*ones(1,200) 9*ones(1,200)]';


%% 

selectid = randperm(classsize,select_data);
selectidfortrain_in = randperm(length(selectid),train_samples);  % Ëæ»úÑ¡Ôñ 80% ×÷ÎªÑµÁ·Êý¾Ý¼¯
selectidfortrain = selectid(selectidfortrain_in);
selectidfortest = selectid;
selectidfortest(selectidfortrain_in) = [];

select_train = [];
select_test = [];
for i=1:numClasses
    select_train = [select_train selectidfortrain + (i-1)*classsize];
    select_test = [select_test selectidfortest + (i-1)*classsize];
end
...
cq_ty  = label(select_test);

fprintf('prepare data for train and test .. \n');
trainLabels = full(sparse(cq_try, 1:train_samples*numClasses, 1));   % 15 * 2000  , ruguo yao 2000*15 jiu xuyao zhuanzhi
testLabels = full(sparse(cq_ty, 1:test_samples*numClasses, 1));

cq_trx = Dataset_CM55(select_train,:)';
lss_trx = Dataset_CH(select_train,:)';
phog_trx = Dataset_CORR(select_train,:)';
sift_trx = Dataset_WT(select_train,:)';
surf_trx = Dataset_EDH(select_train,:)';

cq_tx = Dataset_CM55(select_test,:)';
lss_tx = Dataset_CH(select_test,:)';
phog_tx = Dataset_CORR(select_test,:)';
sift_tx = Dataset_WT(select_test,:)';
surf_tx = Dataset_EDH(select_test,:)';

% ------------------------------------------------------------------------
fprintf('feature normalize ... \n');
% normalize
...
cq_trainData = cq_trx;
trainLabels = cq_try;
lss_trainData = lss_trx;
phog_trainData = phog_trx;
sift_trainData = sift_trx;
surf_trainData = surf_trx;
rgsift_trainData = rgsift_trx;
%}
MaxIter = 300;
%%======================================================================
